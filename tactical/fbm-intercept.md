...
在上述 Markdown 结论后，**追加一个轻量 HTML 结果卡片**，让筛选结果与打法更直观。样式规范见 [`html-report-style.md`](../reference/html-report-style.md)。

要点：
- 用 ` ```html ` 代码块包裹，内联 `<style>` 且 `:root` 变量兜底，独立可预览。
- 建议组件：头部（策略名 + 筛选口径 + 达标标签）、KPI 卡片（命中数 / 核心阈值）、候选结果徽章表格、1 个关键分布/趋势图。
- 数值格式化并注意 `0~1` 刻度换算；图表用 `[[chart]]` ECharts JSON，无外部 CDN。
- 页脚注明数据来源接口与数据月份。
